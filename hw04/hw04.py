...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_mobile(m) or is_planet(m),"the datatype is not correct"
    if(is_planet(m)):
        return True
    leftArm = left(m)
    rightArm = right(m)
    lt = length(leftArm) * total_weight(end(leftArm))
    rt = length(rightArm) * total_weight(end(rightArm))


  
    return lt == rt and balanced(end(leftArm)) and balanced(end(rightArm))
...
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    assert is_mobile(m) or is_planet(m),"must be proper datatype"
    if is_planet(m):
        return tree(size(m))
    else:
